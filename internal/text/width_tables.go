// Code generated by asking vim 9.2.0321 for strdisplaywidth. DO NOT EDIT BY HAND.
//
// Every code point from U+0001 to U+10FFFF was measured as
// strdisplaywidth("a" . nr2char(c)) - 1 under
// vim --clean -es -i NONE --not-a-term, and the runs where the answer was not 1
// are the three tables below. Surrogates are skipped: no valid utf-8 file holds
// one and Go's decoder never hands one back.
//
// Asking vim rather than reading EastAsianWidth.txt is the whole point. Vim
// carries its own emoji width table on top of East Asian Width, so the two
// disagree on 167 code points (U+261D, U+270C, U+1F1E6..U+1F1FF, U+1F441 and
// the rest of the emoji-presentation-variable set), and this package exists to
// return the column vim's virtcol() returns. When Unicode and vim disagree,
// vim wins here by definition.
//
// The same sweep is what internal/screen/width.go is built from, so the two
// tables agree on everything they both cover. They differ on purpose in one
// place: screen draws an unprintable format-control as a single cell and leaves
// the <200b> expansion to the window renderer, while this package has to report
// the six cells vim's virtcol() counts.
//
// golang.org/x/text has similar tables and is not a dependency of this module,
// on purpose: the whole binary is three pinned third-party packages and a width
// table is 500 lines of data, not a reason for a fourth.

package text

// rrange is an inclusive range of code points. The tables below are sorted and
// non-overlapping, so lookup is a binary search.
type rrange struct {
	lo, hi rune
}

// wideRanges holds every code point vim draws in two cells: CJK ideographs,
// Hangul, kana, fullwidth forms, the Unicode 9 emoji blocks, and the
// emoji-presentation code points vim widens that East Asian Width calls
// neutral. The private-use run at U+100000 is vim's answer too, odd as it
// looks, and it is here so a cursor in a file holding one lands where vim's
// does.
var wideRanges = [...]rrange{
	{0x1100, 0x115F}, {0x231A, 0x231B}, {0x2329, 0x232A}, {0x23E9, 0x23F3},
	{0x23F8, 0x23FA}, {0x24C2, 0x24C2}, {0x25FD, 0x25FE}, {0x2614, 0x2615},
	{0x261D, 0x261D}, {0x2630, 0x2637}, {0x2648, 0x2653}, {0x267F, 0x267F},
	{0x268A, 0x268F}, {0x2693, 0x2693}, {0x26A1, 0x26A1}, {0x26AA, 0x26AB},
	{0x26BD, 0x26BE}, {0x26C4, 0x26C5}, {0x26C8, 0x26C8}, {0x26CE, 0x26CF},
	{0x26D1, 0x26D1}, {0x26D3, 0x26D4}, {0x26E9, 0x26EA}, {0x26F0, 0x26F3},
	{0x26F5, 0x26F5}, {0x26F7, 0x26FA}, {0x26FD, 0x26FD}, {0x2705, 0x2705},
	{0x270A, 0x270D}, {0x2728, 0x2728}, {0x274C, 0x274C}, {0x274E, 0x274E},
	{0x2753, 0x2755}, {0x2757, 0x2757}, {0x2795, 0x2797}, {0x27B0, 0x27B0},
	{0x27BF, 0x27BF}, {0x2934, 0x2935}, {0x2B1B, 0x2B1C}, {0x2B50, 0x2B50},
	...
	{0x1F170, 0x1F189}, {0x1F18E, 0x1F18E}, {0x1F191, 0x1F19A},
	{0x1F1E6, 0x1F202}, {0x1F210, 0x1F23B}, {0x1F240, 0x1F248},
	{0x1F250, 0x1F251}, {0x1F260, 0x1F265}, {0x1F300, 0x1F321},
	...
	{0x30000, 0x3FFFD}, {0x100000, 0x1018C7},
}

// zeroRanges holds every code point that draws no cell of its own: the
// nonspacing and enclosing marks, the variation selectors, and the handful of
// letters vim treats the same way. A run of these attaches to the character in
// front of it; charAt is where that happens.
var zeroRanges = [...]rrange{
	...
	{0x10EAB, 0x10EAC}, {0x10EFC, 0x10EFF}, {0x10F46, 0x10F50},
...
// unprintableRanges holds the code points vim refuses to draw and shows as
// <200b> instead, six cells wide. They are the bidi and format controls, the
// zero-width space, the byte-order mark and the non-characters at the end of
// the BMP: exactly the set a file picks up from a web paste, which is why a
// virtcol() that ignored them would be wrong on the files it matters for.
var unprintableRanges = [...]rrange{
